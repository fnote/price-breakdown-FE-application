...
const handleUnknownError = (userDetailContext) => {
    if (!auth.shouldFetchUserDetailsAgain(userDetailContext)) {
        openNotificationWithIcon('error', CIPZErrorMessages.UNKNOWN_ERROR_OCCURRED, CIPZErrorMessages.FETCH_TRANSACTED_BUSINESS_UNITS_ERROR_TITLE);
    }
};
...
const handleError = (userDetailContext) => {
    if (!auth.shouldFetchUserDetailsAgain(userDetailContext)) { // Check this is correct after auth is working
        openNotificationWithIcon('error', CIPZErrorMessages.FETCH_TRANSACTED_BUSINESS_UNITS_ERROR_MESSAGE, CIPZErrorMessages.FETCH_TRANSACTED_BUSINESS_UNITS_ERROR_TITLE);
    }
};
